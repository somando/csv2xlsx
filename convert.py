...
if len(args) < 2 or len(args) > 3 or (len(args) == 2 and (args[1] == '-h' or args[1] == '--help' or args[1].split('.')[-1].lower() != 'csv')) or (len(args) == 3 and (args[1].split('.')[-1].lower() != 'csv')):
    print('使い方: python convert.py <csvファイル名> <出力Excelファイル名 *任意>')
    sys.exit(1)
...
if len(args) == 2:
    ...
    if args[2].split('.')[-1].lower() != 'xlsx':
        for i, name in enumerate(args[2].split('.')):
            if i != len(args[2].split('.')) - 1:
                file_name += name + '.'
            else:
                file_name += name
                break
...
try:
    ...
    print(f'{file_name}.xlsx が開かれているため上書きできません。\nExcelファイルを閉じるか、出力するファイル名を変更してください。')
    sys.exit(1)
